...
#include "trunk_protos_cpp/common/act_status.pb.h"
#include <google/protobuf/text_format.h>

using namespace std;
using namespace std::chrono;

const string SERVER_ADDRESS	{ "tcp://47.93.27.122:1883" };
const string CLIENT_ID		{ "paho_cpp_sync_consume_wyq" };

/////////////////////////////////////////////////////////////////////////////

int main(int argc, char* argv[])
{
  mqtt::client cli(SERVER_ADDRESS, CLIENT_ID); // 实例化mqtt client 对象
  ...
  const vector<string> TOPICS { "+/from/v2/act_status/notify/+", "+/from/v2/task/request/+" };
  ...
  try {
    ...
    while (true) {
      ...
      if (msg) {
        ...
        size_t pos = msg->get_topic().find("/from/v2/act_status/notify/");
        if (pos != string::npos){
          std::cout << "Substring found at position: " << pos << std::endl;
          common::ActStatus act_state;
          act_state.ParseFromString(msg->to_string());
          string str1;
          google::protobuf::TextFormat::PrintToString(act_state, &str1);
          std::cout << "act_state: " << str1 << std::endl;
        }
